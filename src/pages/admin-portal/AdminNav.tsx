...
const AdminNav = ({
  ...
  const isDashboard = nav === Nav.DASHBOARD;
  const isSetting = nav === Nav.SETTING;
            ...
            <div className="flex items-center justify-between -m-2">
              <div className="flex flex-wrap items-center w-auto p-2">
                ...
                <ul className=" xl:flex flex-wrap">
                  <li className="mr-8">
                    <a
                      className={`flex flex-wrap items-center py-8 text-base cursor-pointer font-medium text-coolGray-500 ${
                        isDashboard ? "text-green-500" : "text-gray-500"
                      } border-b-2 ${
                        isDashboard ? "border-green-500" : "border-gray-500"
                      }`}
                      onClick={() => setNav(Nav.DASHBOARD)}
                    >
                      <svg
                        className="mr-2"
                        width={24}
                      ...
                      <p className="text-coolGray-800 cursor-pointer">
                        Admin Dashboard
                      </p>
                    </a>
                  </li>
                  <li className="mr-8">
                    <a
                      className={`flex flex-wrap items-center cursor-pointer py-8 text-base font-medium text-coolGray-500 ${
                        isSetting ? " text-green-500" : " text-gray-500"
                      } border-b-2 ${
                        isSetting ? "border-green-500" : "border-gray-500"
                      }`}
                      onClick={() => setNav(Nav.SETTING)}
                    >
                      <Settings />
                      <p className="ml-2 text-coolGray-800">Settings</p>
                    </a>
                  </li>
                </ul>
              </div>
